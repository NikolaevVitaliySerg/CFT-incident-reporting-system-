...
view VW_CRIT_S16_PROJECT_REQUEST {
    type main is
        select a(a.[NUMBERPROJECT]				: C_NUMBERPROJECT
        		...
        		,(select x(SUM(x.[PLANNEDLABOUR])) 
				in ::[S16_REQUEST] all where 
				x.[PROJECT_ID] = a) 		: C_ACTUALLABOUR
        		,a.[MANAGER_ID].[NAME]			: C_MANAGER)
        in ::[S16_PROJECT] where
        a.[DATECREATION] >= nvl(to_date(sys_context(user_context,'P_DATECREATION'), 'dd.mm.yyyy'), a.[DATECREATION]);
}
